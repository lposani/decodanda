...
class DisentangledGaussian:
    def __init__(self, n_neurons, coding_fraction=0.1, identity_distance=1., position_distance=1., variance=1.):
        from sklearn.datasets import make_spd_matrix

        self.interacting_position = []
        self.interacting_mouse = []
        self.shift = []
        self.raster = []
        self.time = []
        self.x = []
        self.y = []
        self.region_mouse = []
        self.mouse_name = 'Freddie Hubbard'
        self.n_neurons = n_neurons
        self.f = coding_fraction
        self.n_id = int(n_neurons / 2)
        self.n_pos = n_neurons - self.n_id - 1
        self.mouse_names = ['1', '2']
        # define two multivariate gaussians as generative models for the two conditions of position (L-R)
        self.means_L = np.random.rand(self.n_pos) * position_distance
        self.means_R = np.random.rand(self.n_pos) * position_distance
        self.cov_L = np.diag(np.ones(self.n_pos) * variance)
        self.cov_R = self.cov_L

        # define two multivariate gaussians as generative models for the two conditions of identity (1-2)
        self.means_1 = np.random.rand(self.n_id) * identity_distance
        self.means_2 = np.random.rand(self.n_id) * identity_distance
        self.cov_1 = np.diag(np.ones(self.n_id) * variance)
        self.cov_2 = np.diag(np.ones(self.n_id))

        # define rotation matrix
        from scipy.stats import unitary_group
        self.rotation_matrix = unitary_group.rvs(self.n_neurons)

    def __getitem__(self, item):
        return getattr(self, item)

    def sample_activity(self, ndata, persistence_identity=0.95, persistence_position=0.95, rotate=False,
                        displacement=0.0, global_shift=0, append=False):

        raster = np.zeros((ndata, self.n_neurons))
        time = np.arange(len(self.time) - 1, len(self.time) + ndata - 1)
        interacting_mouse = np.zeros(ndata, dtype=object)
        interacting_position = np.zeros(ndata, dtype=object)
        shift = np.zeros(ndata)

        # fill raster array and behavioral arrays with generated data
        interacting_mouse[0] = '1'
        interacting_position[0] = 'left'

        for i in range(1, ndata):
            vector = np.zeros(self.n_neurons)

            if np.random.rand() < persistence_identity:
                if interacting_mouse[i - 1] == '1':
                    vector[0:self.n_id] = np.random.multivariate_normal(self.means_1, self.cov_1, 1)
                    interacting_mouse[i] = '1'
                if interacting_mouse[i - 1] == '2':
                    vector[0:self.n_id] = np.random.multivariate_normal(self.means_2, self.cov_2, 1)
                    interacting_mouse[i] = '2'
            else:
                if interacting_mouse[i - 1] == '2':
                    vector[0:self.n_id] = np.random.multivariate_normal(self.means_1, self.cov_1, 1)
                    interacting_mouse[i] = '1'
            ...
            if np.random.rand() < persistence_position:
                if interacting_position[i - 1] == 'left':
                    vector[self.n_id:-1] = np.random.multivariate_normal(self.means_L, self.cov_L, 1)
                    interacting_position[i] = 'left'
                if interacting_position[i - 1] == 'right':
                    vector[self.n_id:-1] = np.random.multivariate_normal(self.means_R, self.cov_R, 1)
                    interacting_position[i] = 'right'
            else:
                if interacting_position[i - 1] == 'right':
                    vector[self.n_id:-1] = np.random.multivariate_normal(self.means_L, self.cov_L, 1)
                    interacting_position[i] = 'left'
            ...
            vector[-1] = np.random.normal(loc=global_shift)
            raster[i] = vector
            shift[i] = global_shift

        # rotate the array
        if rotate:
            for i in range(len(raster)):
                raster[i, :] = np.dot(raster[i, :], self.rotation_matrix)

        # displace individual blobs
        if displacement:
            self.displace_blobs(raster, interacting_position, interacting_mouse, displacement)

        # add non-linearity by thresholding the raster
        if self.f:
            for n in range(self.n_neurons):
                threshold = np.percentile(np.abs(raster[:, n]), 100 * (1 - self.f))
                raster[np.abs(raster[:, n]) < threshold, n] = 0
                raster[np.abs(raster[:, n]) >= threshold, n] = 1

        # visualization stuff
        x = interacting_position == 'right'
        y = np.zeros(ndata)
        region_mouse = interacting_mouse

        if append:
            self.raster = np.vstack([self.raster, raster])
            self.interacting_mouse = np.hstack([self.interacting_mouse, interacting_mouse])
            self.interacting_position = np.hstack([self.interacting_position, interacting_position])
            self.shift = np.hstack([self.shift, shift])
            self.time = np.hstack([self.time, time])
            self.x = np.hstack([self.x, x])
            self.y = np.hstack([self.y, y])
            self.region_mouse = np.hstack([self.region_mouse, region_mouse])
        else:
            self.raster = raster
            self.interacting_mouse = interacting_mouse
            self.interacting_position = interacting_position
            self.shift = shift
            self.time = time
            self.x = x
            self.y = y
            self.region_mouse = region_mouse

        return self.raster

    def displace_blobs(self, raster, interacting_position, interacting_mouse, gamma):
        for identity in ['1', '2']:
            for position in ['left', 'right']:
                displacement = gamma * (np.random.rand(self.n_neurons) - 0.5)
                mask = (interacting_position == position) & (interacting_mouse == identity)
                raster[mask, :] = raster[mask, :] + displacement


def generate_synthetic_data2(conditions, n_neurons=300, n_trials=200, distortion=0, distance_1=1.0,
                            distance_2=1.0, rotated=False, noise_amplitude=0, noise_skew=1.0):
    session = {}
    for key in conditions:
        session[key] = []

    dg = DisentangledGaussian(n_neurons=n_neurons,
                              identity_distance=distance_1,
                              position_distance=distance_2,
                              coding_fraction=0)

    dg.sample_activity(n_trials, displacement=distortion, rotate=rotated)

    # adding skewed random noise to the raster
    if noise_amplitude:
        noise_factor = np.random.rand(n_neurons) ** noise_skew
        noise = np.random.randn(n_trials, n_neurons) * noise_factor
        dg.raster += noise

    session['raster'] = dg.raster
    session['trial'] = np.arange(len(session['raster']))

    dg.interacting_mouse[dg.interacting_mouse == '1'] = conditions[list(conditions.keys())[0]][0]
    dg.interacting_mouse[dg.interacting_mouse == '2'] = conditions[list(conditions.keys())[0]][1]
    session[list(conditions.keys())[0]] = dg.interacting_mouse

    dg.interacting_position[dg.interacting_position == 'left'] = conditions[list(conditions.keys())[1]][0]
    dg.interacting_position[dg.interacting_position == 'right'] = conditions[list(conditions.keys())[1]][1]
    session[list(conditions.keys())[1]] = dg.interacting_position

    return session


def generate_poisson_disentangled(n_neurons, n_trials, signal=1, mixing_factor=0, scale=1, label_A='A', label_B='B'):
    # sample behavior
    behavior_A = 2*(np.random.rand(n_trials) < 0.5) - 1
    behavior_B = 2*(np.random.rand(n_trials) < 0.5) - 1

    # sample neural parameters
    rates = np.random.lognormal(1, scale, n_neurons)
    rA = np.random.randn(n_neurons) * signal
    rB = np.random.randn(n_neurons) * signal
    ...
    # # mixing_factor between 0 (categorical) and 0.5 (mixed)
    # # define which neurons respond to which variable
    # A_neurons = np.random.rand(n_neurons) < proportion_A
    #
    # # sample mean activation for each class
    # mfr = cauchy.rvs(scale=scale, size=n_neurons)
    # mix = np.random.randn(n_neurons)*mixing_factor
    #
    # mfr[mfr > 10] = 0
    # mfr[mfr < -10] = 0
    #
    #
    # # sample neural response
    # raster = []
    # for n in range(n_neurons):
    #     if A_neurons[n]:
    #         x = np.zeros(n_trials)
    #         x[behavior_A & behavior_B] = np.random.randn(np.sum(behavior_A & behavior_B)) * mfr[n] * (
    #                     1 + mix[n]) + mfr[n] * (1 + mix[n])
    #         x[behavior_A & ~behavior_B] = np.random.randn(np.sum(behavior_A & ~behavior_B)) * mfr[n] * (
    #                     1 - mix[n]) + mfr[n] * (1 - mix[n])
    #         x[~behavior_A & behavior_B] = np.random.randn(np.sum(~behavior_A & behavior_B)) * mfr[n] * (
    #                     -1 + mix[n]) + mfr[n] * (-1 + mix[n])
...
session = generate_synthetic_data(n_trials=20, rateA=0.1, rateB=0.1, meanfr=0.005)
visualize_synthetic_data(session)
conditions = {'stimulus': [-1, 1], 'action': [-1, 1]}
Decodanda(session, conditions, verbose=True).decode(training_fraction=0.75, plot=True)
Decodanda(session, conditions, verbose=True).CCGP(plot=True)
